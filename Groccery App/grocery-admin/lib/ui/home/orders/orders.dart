      ...
      child: Consumer<OrdersModel>(
        builder: (context, model, _) {
          ...
          return Provider<OrdersReaderBloc>(
            create: (context) =>
                OrdersReaderBloc(database: database),
            child: Orders._(
              model: model,
            ),
          );
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
                ...
                onPressed: () {
                  DateFilter.create(context, date: widget.model.date).then((date) {
                    if (date != null) {
                      if (date == "ALL") {
                        widget.model.changeDateRange(context,status ?? 'Processing');
                      } else {
                        widget.model.changeDateRange(context,status ?? 'Processing',date);
                      }
                    }
                  });
                })
